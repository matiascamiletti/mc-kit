...
@Component({
  selector: 'mc-odata-signal-table',
  standalone: true,
  template: '',
})
export abstract class MCOdataSignalTableComponent<T extends { id?: any }> implements OnInit {
  ...
  items = signal<MCListResponse<T>>(new MCListResponse<T>());
